...
budget_limits := {
  "Finance-Controlled": {"transaction_limit": 10000, "approval_threshold": 2000},
  "Sales-Refunds": {"transaction_limit": 5000, "approval_threshold": 500},
  "Operations-Standard": {"transaction_limit": 2500, "approval_threshold": 1200},
  "IT-Restricted": {"transaction_limit": 800, "approval_threshold": 300}
}
...
deny_reason[reason] {
  not allowed_service
  reason := sprintf("%s is not allowed for this Agent Passport.", [input.normalizedExecution.service])
}

deny_reason[reason] {
  not allowed_operation
  reason := sprintf("%s is not allowed for this Agent Passport.", [input.normalizedExecution.operation])
}
...
deny_reason[reason] {
  contains(lower(input.normalizedExecution.parameterText), "bypass approval")
...
deny_reason[reason] {
  input.normalizedExecution.amount > budget_limits[input.identity.budgetProfile].transaction_limit
  reason := sprintf("Amount %.2f exceeds transaction limit.", [input.normalizedExecution.amount])
}
...
approval_reason[reason] {
  ...
  input.normalizedExecution.amount >= budget_limits[input.identity.budgetProfile].approval_threshold
...
decision := {
  ...
  "reasons": ["OPA policy allowed governed enterprise execution."]
} {
  count(deny_reasons) == 0
  count(approval_reasons) == 0
}
