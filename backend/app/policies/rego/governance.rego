...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount > 1000.0
  policy := {"decision": "DENY", "policy_id": "budget_it_restricted_transaction_limit", "priority": 899980, "reason": "IT-Restricted transaction limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount + 0.0 > 2500.0
  policy := {"decision": "DENY", "policy_id": "budget_it_restricted_daily_limit", "priority": 899979, "reason": "IT-Restricted daily limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount + 0.0 > 20000.0
  policy := {"decision": "DENY", "policy_id": "budget_it_restricted_monthly_limit", "priority": 899978, "reason": "IT-Restricted monthly limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount >= 500.0
  policy := {"decision": "REQUIRE_APPROVAL", "policy_id": "budget_it_restricted_approval_threshold", "priority": 899977, "reason": "IT-Restricted approval threshold reached."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount > 4000.0
  policy := {"decision": "DENY", "policy_id": "budget_operations_standard_transaction_limit", "priority": 899970, "reason": "Operations-Standard transaction limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount + 0.0 > 12000.0
  policy := {"decision": "DENY", "policy_id": "budget_operations_standard_daily_limit", "priority": 899969, "reason": "Operations-Standard daily limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount + 0.0 > 100000.0
  policy := {"decision": "DENY", "policy_id": "budget_operations_standard_monthly_limit", "priority": 899968, "reason": "Operations-Standard monthly limit exceeded."}
}
...
policy_matches contains policy if {
  ...
  amount := input.normalizedExecution.amount
  amount >= 1500.0
  policy := {"decision": "REQUIRE_APPROVAL", "policy_id": "budget_operations_standard_approval_threshold", "priority": 899967, "reason": "Operations-Standard approval threshold reached."}
}
